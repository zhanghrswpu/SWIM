    ...
    if partition == "homo" or partition == "iid":
        ...
        while min_size < min_require_size:
            ...
            for k in range(K):
                ...
                #         min_size = 0
                #         break
                # net_dataidx_map = [idx_j + idx.tolist() for idx_j, idx in zip(idx_batch, np.split(idx_k, proportions))]
                # net_dataidx_map=idx_batch
        for j in range(n_parties):
            np.random.shuffle(idx_batch[j])
            net_dataidx_map[j] = idx_batch[j]
        # net_dataidx_map = {j: idx_batch[j] for j in range(n_parties)}

    # print("canshu:", net_dataidx_map[2])
